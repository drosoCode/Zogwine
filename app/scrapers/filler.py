from app.scrapers.interfaces.filler import *
...
class filler(BaseScraper):
    def __init__(self, config):
        super().__init__(FillerProvider, config)
        self._mediaType = 7
        ...
        for p in self._getPersonData():
            if p["scraperID"] is None:
                ...
                for provider in self._getProviders():
                    for i in provider.searchFiller(p["name"]):
    ...
    def _updateWithSelectionResult(
        ...
        sqlConnection, cursor = getSqlConnection()
        cursor.execute(
            "UPDATE fillers SET scraperID = %(scraperID)s, scraperName = %(scraperName)s, scraperData = %(scraperData)s, forceUpdate = 1 WHERE idFiller = %(mediaData)s;",
            {
                "mediaData": mediaData,
                "scraperID": scraperID,
