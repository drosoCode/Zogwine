...
class people:
    ...
    def scan(self):
        ...
        for p in cursor.fetchall():
            ...
            cursor.execute(
                "UPDATE people SET gender = %(gender)s, birthdate = %(birthdate)s, deathdate = %(deathdate)s, description = %(description)s, known_for = %(known_for)s, icon = %(icon)s, forceUpdate = 0 WHERE idPers = %(idPers)s;",
                queryData,
            )
